...
\paragraph{As 10 \textit{sefirót}} são emanações divinas que estruturam \textit{olam}, o ``mundo'', \textit{shaná}, o ``tempo'' e \textit{nefesh}, o ``homem''
%Elas permitem ao \textit{ein sof}, o ``ilimitado'', se tornar mundo. Estão relacionadas aos números de 1 a 10. 
...
% {\pagecolor{black}
% \color{white}
% \paragraph{Epílogo} A realidade, que é feita de relações e combinações, está sempre em transformação. Ao mudar, cria-se algo novo. Cada par de letras representa, portanto, um portão de som e de sentido: a ligação entre duas ideias, dois sons, duas forças. Quando combinadas ou intercambiadas, essas letras geram vibrações ou expressões de princípios cósmicos. E torna-se possível \textit{tocar} o ato criador. 
